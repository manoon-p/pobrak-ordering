...
export default function TablePage() {
  ...
  if (!session) return <TableSelect onSelect={setSession} />
  return <MenuPage session={session} onBack={() => setSession(null)} />
}

function TableSelect({ onSelect }: any) {
  const [selected, setSelected] = useState<number|null>(null)
  const [adults, setAdults] = useState(2)
  const [children, setChildren] = useState(0)
  return (
    <div style={{fontFamily:"sans-serif",minHeight:"100vh",background:"#1b4332",display:"flex",flexDirection:"column",alignItems:"center"}}>
      <div style={{padding:"40px 24px",textAlign:"center",color:"white"}}>
        <div style={{fontSize:48}}>🍽️</div>
        <h1 style={{margin:"10px 0"}}>พบรัก ณ บางน้ำผึ้ง</h1>
      </div>
      <div style={{width:"100%",maxWidth:430,background:"white",borderRadius:"28px 28px 0 0",flex:1,padding:"26px 20px"}}>
        ...
        <button onClick={()=>selected&&onSelect({tableId:selected,adults,children})} disabled={!selected} style={{width:"100%",padding:"17px",background:selected?"#2d6a4f":"#e0e0e0",color:selected?"white":"#aaa",border:"none",borderRadius:16,fontWeight:800,fontSize:17,cursor:selected?"pointer":"not-allowed"}}>
...
function MenuPage({ session, onBack }: any) {
  return (
    <div style={{fontFamily:"sans-serif",minHeight:"100vh",background:"#f0faf4"}}>
      <div style={{background:"#1b4332",padding:"20px",color:"white"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:12,color:"#95d5b2"}}>พบรัก ณ บางน้ำผึ้ง</div>
            <div style={{fontWeight:900,fontSize:22}}>โต๊ะ {session.tableId}</div>
            <div style={{fontSize:13,color:"#b7e4c7"}}>{session.adults} adults</div>
          </div>
          <button onClick={onBack} style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:12,padding:"8px 13px",color:"white",cursor:"pointer"}}>เปลี่ยนโต๊ะ</button>
        </div>
      </div>
      <div style={{padding:20,textAlign:"center"}}>
        <div style={{fontSize:48,marginBottom:16}}>🍽️</div>
        <div style={{fontWeight:800,fontSize:20,color:"#1b4332"}}>เมนูอาหาร</div>
        <div style={{marginTop:16,background:"#d8f3dc",borderRadius:14,padding:16,border:"1.5px solid #95d5b2"}}>
          <div style={{fontWeight:700,color:"#1b4332"}}>Firebase Connected!</div>
          <div style={{fontSize:13,color:"#2d6a4f",marginTop:4}}>โต๊ะ {session.tableId}</div>
        </div>
      </div>
    </div>
  )
